...
static int _fib_action(int *args, size_t size) {
	...
	if (my_data->n < 2)
		HPX_THREAD_CONTINUE(my_data->n);

	struct thread_data temp[2];
	...
	temp[0].n = my_data->n - 1;
	temp[1].n = my_data->n - 2;
	...
	hpx_call(peers[0], _fib, futures[0], &temp[0], sizeof(struct thread_data));
	hpx_call(peers[1], _fib, futures[1], &temp[1], sizeof(struct thread_data));
...
static int _fib_main_action(int *args, size_t size) {
	...
	hpx_call_sync(HPX_HERE, _fib, &fn, sizeof(fn), &temp, sizeof(struct thread_data));
...
int main(int argc, char *argv[]) {
	...
	// run the main action
	int e = hpx_run(&_fib_main, &n, sizeof(int));
	hpx_finalize();
	return e;
}
