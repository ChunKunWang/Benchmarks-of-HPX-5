...
static counter_t _uts_action(void *args, size_t size) 
{
	...
	// declare hpx-5 lco variables
	hpx_addr_t futures[temp.numChildren];
	hpx_addr_t theThread = HPX_HERE;
	...
	for (i = 0; i < temp.numChildren; i++) {
		...
		hpx_call(threads[i], _uts, futures[i], &input, sizeof(input));
	}

	//printf("\n%d\n", temp.numChildren);
	if( temp.numChildren > 0 ) {
		hpx_lco_get_all(temp.numChildren, futures, p_size, addrs, NULL);
