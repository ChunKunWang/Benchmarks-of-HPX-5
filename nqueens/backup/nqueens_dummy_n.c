...
static int _nqueens_action(void *args, size_t size)
{
	...
	struct thread_data temp[my_data->n];
	//hpx_addr_t done = hpx_lco_future_new(sizeof(int));
	hpx_addr_t futures[my_data->n];
	hpx_addr_t threads[my_data->n];
	int pqs[my_data->n];
	int p_size[my_data->n];
	void *addrs[my_data->n];

	for(i = 0; i < my_data->n; i++) {
...
#define p_attack(i, j) (my_data->lyst[j] == i || abs(my_data->lyst[j] - i) == my_data->col - j)
	...
	for(i = 0, j = 0; i < my_data->n; i++) {
		...
		if (j < my_data->col) {
			...
			hpx_call(threads[i], _dummy, futures[i], NULL, 0);
			continue;
		}
		...
		memcpy(temp[i].lyst, my_data->lyst, MAX_SIZE*sizeof(int));
		temp[i].n    = my_data->n;
		temp[i].col  = my_data->col+1;

		//solve(n, col + 1, hist);
		//hpx_addr_t done = hpx_lco_future_new(sizeof(int));
		//hpx_call(HPX_HERE, _nqueens, done, temp, sizeof(temp));
		hpx_call(threads[i], _nqueens, futures[i], (void *)&temp[i], sizeof(temp[i]));
		//hpx_lco_wait(done);
